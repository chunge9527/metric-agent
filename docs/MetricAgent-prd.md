...
* --config：指定启动基础配置文件，例 --config=./metricAgent.yml
...
* 默认基础配置文件 ./metricAgent.yml，支持通过启动参数 --config 自定义配置文件路径，支持绝对路径与相对路径，相对路径指 MetricAgent 所在目录。

* 基础配置文件 metricAgent.yml 必须包含 agent_id 字段，作为当前节点的唯一标识；未配置该字段时启动直接报错，agent_id 只允许字母、数字、下划线和横线。
...
* Nacos 服务地址、默认namespace、默认group、username、password、timeout、notLoadCacheAtStart在本地基础配置文件 metricAgent.yml 中配置；
...
* 加密规则：采用 AES 对称加密算法，加密密钥在 `metricAgent.yml` 中通过 `shell.encrypt.key` 字段独立配置；**请求体内脚本内容、响应体内执行结果（标准输出、标准错误、退出码、执行耗时）加密；鉴权请求头不加密**。拒绝空脚本、纯空白字符脚本。
...
* 守护配置文件固定路径：./crontab.yml（默认为 Agent 工作目录）
...
* VictoriaMetrics 请求地址配置在基础配置文件 metricAgent.yml 中，字段为 victoriametrics.url。
...
* 日志级别读取自 metricAgent.yml `log.level`，默认值 `info`
...
> 3. 指标前缀固定 `metricagent_`；指标类型仅允许：Counter(只增不减) / Gauge(瞬时状态值) / Histogram(耗时分布统计)。
> 4. Histogram统一bucket：`[0.05, 0.1, 0.5, 1, 2, 5, 10, 30, 60]`，单位秒。
> 5. 埋点仅在HTTP服务模式启用；指令执行模式（--exec）不初始化Prometheus指标、不暴露/metrics接口。
> 6. Prometheus metrics接口复用项目现有暴露路径方案，默认`/metrics`。
> 7. 每个Counter必须在所有分支路径（成功、失败、跳过、超时）都执行Inc，不能只在成功分支计数。
> 8. Gauge值统一约定：`1 = 激活/正常/是，0 = 未激活/异常/否`。
